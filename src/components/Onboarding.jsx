...
export function Onboarding({userId,onComplete,onBack}) {
  ...
  const [soloOwner,setSoloOwner]=useState(true);const [hasMort,setHasMort]=useState(false);
  ...
  const finish=async()=>{setBusy(true);try{
    const pl=soloOwner
      ?[{id:'p0',name:prs[0].name||(auth.currentUser?.email?.split('@')[0])||'Propietario',email:'',ownership:100,initialCapital:parseFloat(prs[0].cap)||0,color:C[0]}]
      :prs.map((x,i)=>({id:'p'+i,name:x.name,email:x.email||'',ownership:parseFloat(x.own)||0,initialCapital:parseFloat(x.cap)||0,color:C[i%C.length]}));
    ...
    const d={name:p.name,address:p.address,city:p.city,state:p.state,type:p.type,country:p.country||'US',currency:p.currency||'USD',managedBy:p.managedBy||'pm',purchasePrice:parseFloat(p.price)||0,purchaseDate:p.date,bedrooms:parseInt(p.beds)||0,bathrooms:parseInt(p.baths)||0,manager:p.managedBy==='pm'?p.pm:'',managerCommission:p.managedBy==='pm'?(parseFloat(p.pmFee)||15):0,partners:pl,memberEmails:me,
      mortgage:hasMort?{balance:parseFloat(mt.bal)||0,rate:parseFloat(mt.rate)||0,termYears:parseInt(mt.term)||30,monthlyPayment:parseFloat(mt.pay)||0,startDate:mt.start||''}:{balance:0,rate:0,termYears:30,monthlyPayment:0,startDate:''},ownerId:userId,createdAt:serverTimestamp()};
  ...
  return <div className="min-h-screen bg-[#080E1A] flex items-center justify-center p-4" style={{backgroundImage:'radial-gradient(ellipse at 50% 0%,rgba(37,99,235,.08) 0%,transparent 50%)'}}>
    <div className="w-full max-w-[580px]">
      ...
      <div className="bg-white rounded-3xl shadow-2xl p-7">
        ...
        {step===1&&<div><h2 className="text-lg font-extrabold text-slate-800 mb-1 flex items-center gap-2"><Users size={20} className="text-blue-500"/> Socios</h2><p className="text-sm text-slate-400 mb-4">¿La propiedad es solo tuya o tiene socios?</p>
          <div className="grid grid-cols-2 gap-2 mb-5">{[[true,'👤 Único dueño (yo)'],[false,'👥 Tengo socios']].map(([v,l])=><button key={String(v)} type="button" onClick={()=>setSoloOwner(v)} className={`py-2.5 rounded-xl border-2 text-xs font-medium transition ${soloOwner===v?'border-blue-500 bg-blue-50 text-blue-700':'border-slate-200 text-slate-500'}`}>{l}</button>)}</div>
          {soloOwner&&<div className="rounded-2xl p-4 mb-3 bg-emerald-50 border border-emerald-200 text-sm text-emerald-800">✓ Eres el 100% dueño de esta propiedad. Puedes continuar sin llenar nada más.</div>}
          {!soloOwner&&prs.map((x,i)=><div key={i} className="rounded-2xl p-4 mb-3 bg-slate-50 border-l-4" style={{borderLeftColor:C[i%C.length]}}><div className="flex justify-between items-center mb-3"><span className="text-xs font-extrabold uppercase tracking-widest" style={{color:C[i%C.length]}}>Socio {i+1}</span>{prs.length>1&&<button onClick={()=>rmPr(i)} className="text-slate-300 hover:text-rose-500 p-1 rounded-lg hover:bg-rose-50"><X size={16}/></button>}</div>
          ...
          {!soloOwner&&<div className="flex justify-between items-center mt-3 mb-6"><button onClick={addPr} className="text-sm text-blue-600 font-bold flex items-center gap-2 px-3 py-2 rounded-xl hover:bg-blue-50"><UserPlus size={15}/> Agregar</button><span className={`text-sm font-extrabold px-4 py-1.5 rounded-full ${totOwn===100?'bg-emerald-100 text-emerald-700':'bg-amber-100 text-amber-700'}`}>{totOwn}%{totOwn===100?' ✓':' ≠100%'}</span></div>}
          <div className="flex justify-between"><button onClick={()=>setStep(0)} className="px-5 py-2.5 border-2 border-slate-200 rounded-xl font-semibold text-slate-600 hover:bg-slate-50">← Atrás</button><button onClick={()=>setStep(2)} disabled={!soloOwner&&(!prs[0].name||totOwn!==100)} className="px-7 py-2.5 bg-blue-600 text-white rounded-xl font-bold text-sm disabled:opacity-30 shadow-lg shadow-blue-500/20">Siguiente →</button></div></div>}
        {step===2&&<div><h2 className="text-lg font-extrabold text-slate-800 mb-1 flex items-center gap-2"><Landmark size={20} className="text-blue-500"/> Hipoteca <span className="text-[10px] font-semibold text-slate-400 bg-slate-100 px-2 py-1 rounded-full">OPCIONAL</span></h2><p className="text-sm text-slate-400 mb-4">¿Esta propiedad tiene hipoteca?</p>
          <div className="grid grid-cols-2 gap-2 mb-5">{[[false,'✅ No tiene hipoteca'],[true,'🏦 Sí, tiene hipoteca']].map(([v,l])=><button key={String(v)} type="button" onClick={()=>setHasMort(v)} className={`py-2.5 rounded-xl border-2 text-xs font-medium transition ${hasMort===v?'border-blue-500 bg-blue-50 text-blue-700':'border-slate-200 text-slate-500'}`}>{l}</button>)}</div>
          {!hasMort&&<div className="rounded-2xl p-4 bg-emerald-50 border border-emerald-200 text-sm text-emerald-800">✓ Propiedad sin hipoteca. Puedes crearla directamente.</div>}
          {hasMort&&<div className="space-y-3"><div className="grid grid-cols-1 md:grid-cols-2 gap-3"><Inp label="Balance" value={mt.bal} onChange={v=>um('bal',v)} prefix="$" type="number"/><Inp label="Tasa (%)" value={mt.rate} onChange={v=>um('rate',v)} type="number"/></div>
