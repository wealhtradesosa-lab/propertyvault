  ...
  const { prop, propertyId, partners, latestVal, settingsForm, setSettingsForm, editPartners, setEditPartners, stmts, expenses, income, contribs, valuations, repairs, notify, propTerms, COUNTRIES, CURRENCY_LIST, US, PT, M, fm } = useDashboard();
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 max-w-2xl">
        ...
        <div className="space-y-3">
          ...
          {sf2.rentalType==='traditional'&&<div className="bg-cyan-50 rounded-xl p-4 border border-cyan-200">
            <div className="text-[10px] font-bold text-cyan-700 uppercase mb-2">Cartera y Mora</div>
            <div className="grid grid-cols-2 gap-3">
              <Inp label="Día de vencimiento del canon" value={sf2.rentDueDay} onChange={v=>uf('rentDueDay',v)} type="number" placeholder="5"/>
              <Inp label="Tasa de mora anual (%)" value={sf2.moraRateAnnual} onChange={v=>uf('moraRateAnnual',v)} type="number" placeholder="0"/>
            </div>
            <p className="text-[10px] text-cyan-500 mt-2">El canon vence el día indicado de cada mes. La mora se calcula como interés simple diario sobre el canon vencido (0 = sin mora). El incremento anual del contrato de cada inquilino se aplica automáticamente en su aniversario.</p>
          </div>}
